...
class CrackItem(settings: Settings): AbstractDrugItem(settings) {
    ...
    override fun condition(user: PlayerEntity) =
        user.inventory.contains(Drugs.CRACK_PIPE.defaultStack) && user.inventory.contains(Items.FLINT_AND_STEEL.defaultStack)
    override fun conditionFailedText(user: PlayerEntity): String {
        ...
        if (!hasFlintAndSteel && !user.inventory.contains(Drugs.CRACK_PIPE.defaultStack)) {
